...
| 🌐 **Multilingua** | Italiano e inglese, di default nella lingua di sistema — modificabile in qualsiasi momento dalle impostazioni. |
...
- **Impostazioni** — dal menu `···`. Sono divise in:
  - **Generali** — lingua, tema, trasparenza, intestazione, comportamento della sidebar;
...
Flowcode è disponibile in **italiano** e **inglese**. Di default usa la lingua del sistema operativo
(con l'inglese come ripiego se quella lingua non è disponibile); puoi sceglierne una specifica da
**Impostazioni → Generali → Lingua**. Il cambio si applica subito a tutte le finestre aperte.

Vuoi aggiungere una lingua? Ognuna è un singolo catalogo in `src/i18n/locales/`: copia `en.ts`,
traduci i valori e registrala in `LANGUAGES` in `src/i18n/index.tsx`. I cataloghi sono tipizzati su
quello italiano di riferimento, quindi una chiave mancante è un errore di compilazione.
...
│   ├── i18n/            # Traduzioni (un catalogo per lingua)
